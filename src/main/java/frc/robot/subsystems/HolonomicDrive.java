...
public class HolonomicDrive extends SubsystemBase { 
  ...
  //This value also goes unused for now. I am not sure if we will need to use it even when we use velocity control
  ...
  private final Translation2d frontRightLocation = new Translation2d(Units.inchesToMeters(11), Units.inchesToMeters(-10));
  private final Translation2d backLeftLocation = new Translation2d(Units.inchesToMeters(-11), Units.inchesToMeters(10));
  private final Translation2d backRightLocation = new Translation2d(Units.inchesToMeters(-11), Units.inchesToMeters(-10)); 
  //Our gyro sensor that returns the robot heading. It is plugged into the serial port on the robot
  ...
  public HolonomicDrive() { 
    //So far, resetting the gyro sensor every robot boot-up has not presented any problems
    gyro.reset(); 
  ...
  public void setSpeeds(MecanumDriveWheelSpeeds speeds){ 
    ...
    //Here you can see where the motors are each commanded to a unique percent voltage value defined above
  ...
  //Our drive method. It takes three independent speeds from our joystick and blends them to get one directional speed
  public void drive(double xSpeed, double ySpeed, double rot){ 
   //"var" is not the best way to do this, but the example code did it so I didn't change it  
   var mecanumDriveWheelSpeeds = 
    //Here is where the kinematics come into play
    kinematics.toWheelSpeeds(
        //Depending on the value of this boolean, we will either drive field oriented or robot oriented
        fieldRelative() 
                  ? ChassisSpeeds.fromFieldRelativeSpeeds(xSpeed, ySpeed, rot, 
                  /** I originally had `Rotation2d.fromDegrees(gyro.getAngle())`, but for some reason this affected field
                   oriented strafing. However, the below function works fine. Not sure what is wrong here */
                  gyro.getRotation2d()) 
  ...
  //Checks if we have a gyro sensor plugged in, enabling field oriented if it is and disabling it if it is not 
